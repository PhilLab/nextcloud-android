 ...
 *
 * Nextcloud-maintained copy of submodules/fairscan/app/src/main/java/org/fairscan/app/FairScanApp.kt
 * (see :fairscan/build.gradle.kts for why). Two changes from upstream:
 *  - FairScanApp is `open`, so MainApp can extend it.
 *  - appContainer is built lazily on first access instead of eagerly in onCreate(), so
 *    MainApp's own onCreate() - which every Nextcloud user hits on every cold start - doesn't
 *    unconditionally pay for OCR-service and image-segmentation-model initialization. MainApp
 *    calls super.onCreate() normally; this class's onCreate() no longer does anything beyond that.
 * Diff against upstream after bumping the submodule pin.
 */
...
open class FairScanApp : Application() {
    val appContainer: AppContainer by lazy {
        AppContainer(this).also { it.cleanOrphanSessions() }
    }
}
